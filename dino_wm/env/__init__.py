...
# max_episode_steps matches ManiSkill's own registration for PushCube-v1 (and the
# recorded trajectory length). The env checker is disabled because, like every wrapper
# here, reset() returns (obs, state) rather than the (obs, info) gym expects.
register(
    id="push_cube",
